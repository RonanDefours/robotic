    ...
    if (state == 1){
      /**
        ...
        */
        ...
        /**
          * MD -
          * Si valeur relative et non pas absolue :
          * TODO changer cette valeur et remplacer par
          * position du robot + 50 y (ou x si coord. inversées)
          *
          */
        translation_to_do = 50;

        ROS_INFO("(decision_node) /translation_to_do: %f", translation_to_do);
        std_msgs::Float32 msg_translation_to_do;
        /**
          * MD - On déplace le robot par pas de 50
          */
