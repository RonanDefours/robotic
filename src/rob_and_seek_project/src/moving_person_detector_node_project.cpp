...
void detect_motion() {
    ...
    for (int loop=0; loop<nb_beams; loop++ ){//loop over all the hits
        // the difference between ( the background and the current value ) 
        //is higher than "detection_threshold"
...
//else we start a new cluster with the current hit and end the current cluster
    ...
    for( int loop=1; loop<nb_beams; loop++ )//loop over all the hits
        //distance between (the previous hit and the current one) is lower than "cluster_threshold"
        if ((distancePoints(current_scan[loop-1], current_scan[loop]))<=cluster_threshold){
        // then //the current hit belongs to the current cluster
            cluster[nb_cluster] = loop;
            if(loop == nb_beams){
                cluster_end[nb_cluster] = loop;
                float nb_point = 0;
                for(int point = cluster_start[nb_cluster]; point<=cluster_end[nb_cluster]; point++){
                    nb_point += (float)dynamic[point];
                }
                cluster_dynamic[nb_cluster] = (int)((nb_point/(cluster_end[nb_cluster]-(cluster_start[nb_cluster]+1)))*100);

                float distance_cluster = sqrt(pow(current_scan[cluster_end[nb_cluster]].x - current_scan[cluster_start[nb_cluster]].x,2.0)+pow(current_scan[cluster_end[nb_cluster]].y - current_scan[cluster_start[nb_cluster]].y ,2.0)+ pow(current_scan[cluster_end[nb_cluster]].z - current_scan[cluster_start[nb_cluster]].z ,2.0));
                cluster_size[nb_cluster] = distance_cluster;

                geometry_msgs::Point middle;
                middle.x =  (current_scan[cluster_end[nb_cluster]].x - current_scan[cluster_start[nb_cluster]].x)/2;
                middle.y =  (current_scan[cluster_end[nb_cluster]].y - current_scan[cluster_start[nb_cluster]].y)/2;
                middle.z =  (current_scan[cluster_end[nb_cluster]].z - current_scan[cluster_start[nb_cluster]].z)/2;
            ...
            float distance_cluster = sqrt(pow(current_scan[cluster_end[nb_cluster]].x - current_scan[cluster_start[nb_cluster]].x,2)+pow(current_scan[cluster_end[nb_cluster]].y - current_scan[cluster_start[nb_cluster]].y ,2)+ pow(current_scan[cluster_end[nb_cluster]].z - current_scan[cluster_start[nb_cluster]].z ,2));
            cluster_size[nb_cluster] = distance_cluster;
...
// - more than "dynamic_threshold"% of its hits are dynamic (see, cluster_dynamic table)
    ...
    for (int loop=0; loop<nb_cluster; loop++){//loop over all the clusters
        // the size of the current cluster is higher than "leg_size_min" and lower than "leg_size_max" and it has "dynamic_threshold"% of its hits that are dynamic
...
// a moving person has two moving legs located at less than "legs_distance_max" one from the other
    ...
    for (int loop_leg1=0; loop_leg1<nb_moving_legs_detected; loop_leg1++){//loop over all the legs
        for (int loop_leg2=loop_leg1+1; loop_leg2<nb_moving_legs_detected; loop_leg2++){//loop over all the legs
            //the distance between two moving legs is lower than "legs_distance_max"
            ...
            if((distancePoints(moving_leg_detected[loop_leg1], moving_leg_detected[loop_leg2]))<legs_distance_max){ 
                ...
                geometry_msgs::Point middle;
                middle.x = (moving_leg_detected[loop_leg1].x + moving_leg_detected[loop_leg2].x)/2;
                middle.y = (moving_leg_detected[loop_leg1].y + moving_leg_detected[loop_leg2].y)/2;
                middle.z = (moving_leg_detected[loop_leg1].z + moving_leg_detected[loop_leg2].z)/2;
                
                moving_persons_detected[nb_moving_persons_detected] = middle;
